...
def test_idempotent_event_recording(db_session: Session) -> None:
    repo = ExecutionHistoryRepository()
    e1 = _create_sample_event(event_id="evt-dup", duration_ms=100.0)
    repo.record_event(db_session, e1)
    db_session.commit()

    # Update duplicate event id
    e1_updated = _create_sample_event(event_id="evt-dup", duration_ms=200.0)
    repo.record_event(db_session, e1_updated)
    db_session.commit()

    rec = repo.get_event_by_id(db_session, "evt-dup")
    assert rec is not None
